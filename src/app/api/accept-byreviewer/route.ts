...
import { z } from 'zod';
import { acceptByReviewerMock } from '@/mock/data/review-actions';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await acceptByReviewerMock(documentId, reviewerNotes);
    ...
    console.error('Accept By Reviewer API Error:', error);
    return NextResponse.json(
      { success: false, message: 'Failed to approve document' },
